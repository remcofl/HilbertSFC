...
def gray2binary(gray, axis=-1):
    ...
    while shift > 0:
        gray = torch.logical_xor(gray, right_shift(gray, int(shift)))
...
def encode(locs, num_dims, num_bits):
    ...
    if orig_shape[-1] != num_dims:
        raise ValueError(
            "The shape of locs was surprising in that the last dimension was "
            f"of size {orig_shape[-1]}, but num_dims={num_dims}. These need "
            "to be equal."
        )

    if num_dims * num_bits > 64:
        raise ValueError(
            f"num_dims={num_dims} and num_bits={num_bits} for "
            f"{num_dims * num_bits} bits total, which can't be encoded into "
            "a 64-bit value. Are you sure you need that many points on your "
            "Hilbert curve?"
        )
...
def decode(hilberts, num_dims, num_bits):
    ...
    if num_dims * num_bits > 64:
        raise ValueError(
            f"num_dims={num_dims} and num_bits={num_bits} for "
            f"{num_dims * num_bits} bits total, which can't be encoded into "
            "a uint64. Are you sure you need that many points on your "
            "Hilbert curve?"
        )
