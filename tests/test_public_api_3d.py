...
def test_3d_scalar_roundtrip_bruteforce(small_nbits_3d: tuple[int, ...]) -> None:
    for nbits in small_nbits_3d:
        ...
        for x in range(size):
            for y in range(size):
                for z in range(size):
                    idx = hilbert_encode_3d(x, y, z, nbits)
                    assert isinstance(idx, int)
                    assert 0 <= idx < (1 << (3 * nbits))

                    x2, y2, z2 = hilbert_decode_3d(idx, nbits)
                    assert (x2, y2, z2) == (x, y, z)


def test_3d_scalar_invalid_nbits_raises() -> None:
    with pytest.raises(ValueError):
        hilbert_encode_3d(0, 0, 0, 0)
    with pytest.raises(ValueError):
        hilbert_decode_3d(0, 22)
...
def test_3d_scalar_out_rejected_and_parallel_warns() -> None:
    ...
    with pytest.raises(TypeError, match="out is only valid"):
        hilbert_encode_3d(0, 0, 0, 2, out=out)  # type: ignore[arg-type]

    with pytest.warns(UserWarning, match="parallel=True has no effect"):
        hilbert_encode_3d(0, 0, 0, 2, parallel=True)  # type: ignore[reportArgumentType]

    with pytest.warns(UserWarning, match="parallel=True has no effect"):
        hilbert_decode_3d(0, 2, parallel=True)  # type: ignore[reportArgumentType]
...
def test_3d_batch_roundtrip_with_lut_dtype_and_parallel(
    ...
    idx = hilbert_encode_3d(xs, ys, zs, nbits, parallel=True)
    assert idx.shape == (n,)
    # 3*6=18 bits -> uint32
    assert idx.dtype == np.dtype(np.uint32)

    x2, y2, z2 = hilbert_decode_3d(idx, nbits, parallel=True)
...
def test_3d_decode_batch_out_triple_rule(rng: np.random.Generator) -> None:
    ...
    idx = hilbert_encode_3d(xs, ys, zs, nbits)

    out_xs = np.empty(n, dtype=np.uint8)
    out_ys = np.empty(n, dtype=np.uint8)

    with pytest.raises(ValueError, match="provided together"):
        hilbert_decode_3d(idx, nbits, out_xs=out_xs, out_ys=out_ys)  # type: ignore
...
def test_3d_batch_coord_dtype_validation() -> None:
    ...
    with pytest.raises(ValueError, match="coordinate dtype"):
        hilbert_encode_3d(xs, ys, zs, 9)
...
def test_3d_decode_batch_out_coord_dtype_validation() -> None:
    ...
    with pytest.raises(ValueError, match="coordinate dtype"):
        hilbert_decode_3d(idx, 9, out_xs=out_xs, out_ys=out_ys, out_zs=out_zs)
