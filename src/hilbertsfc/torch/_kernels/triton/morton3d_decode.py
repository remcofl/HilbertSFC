...
@triton.jit
def _compact1by2_u32(x: tl.tensor, NBITS: tl.constexpr) -> tl.tensor:
    x = x & 0x09249249
    if NBITS > 1:
        x = (x ^ (x >> 2)) & 0x030C30C3
    if NBITS > 2:
...
@triton.jit
def _compact1by2_u64(x: tl.tensor, NBITS: tl.constexpr) -> tl.tensor:
...
def morton_decode_3d_kernel(
    ...
    IDX_MASK: tl.constexpr = (1 << (3 * NBITS)) - 1 if NBITS < 21 else 0  # type: ignore[reportAssignmentType, reportOperatorIssue]
    ...
    if NBITS <= 10:
        ...
        x = _compact1by2_u32(idx, NBITS)
        y = _compact1by2_u32(idx >> 1, NBITS)
        z = _compact1by2_u32(idx >> 2, NBITS)
    else:
        if NBITS < 21:
            idx = idx & IDX_MASK
        idx = idx.to(tl.uint64)
        x = _compact1by2_u64(idx, NBITS)
        y = _compact1by2_u64(idx >> 1, NBITS)
        z = _compact1by2_u64(idx >> 2, NBITS)
...
morton_decode_3d_kernel_autotuned = triton.autotune(
    configs=triton_autotune_configs(),
    key=["AUTOTUNE_KEY", "NBITS"],
)(morton_decode_3d_kernel)
...
def morton_decode_3d_triton(
    ...
    *,
    ...
    """Decode Morton indices to (x, y, z) using Triton."""
    ...
    if triton_tuning == "heuristic":
        ...
        morton_decode_3d_kernel[grid](  # type: ignore[reportIndexIssue]
            index,
            out_x,
            out_y,
            out_z,
    ...
    morton_decode_3d_kernel_autotuned[grid](  # type: ignore[reportIndexIssue]
        index,
        out_x,
        out_y,
        out_z,
