...
@triton.jit
def _part1by1_u32(x: tl.tensor, NBITS: tl.constexpr) -> tl.tensor:
    if NBITS > 8:
        x = (x | (x << 8)) & 0x00FF00FF
    if NBITS > 4:
        x = (x | (x << 4)) & 0x0F0F0F0F
...
@triton.jit
def _part1by1_u64(x: tl.tensor, NBITS: tl.constexpr) -> tl.tensor:
    if NBITS > 16:
...
def morton_encode_2d_kernel(
    ...
    if NBITS <= 16:
        x = x.to(tl.uint32)
        y = y.to(tl.uint32)
        idx = _part1by1_u32(x, NBITS) | (_part1by1_u32(y, NBITS) << 1)
        tl.store(out_ptr + offsets, idx, mask=mask)
    else:
        x = x.to(tl.uint64)
        y = y.to(tl.uint64)
        idx = tl.zeros([BLOCK_SIZE], dtype=OUT_DTYPE)
        idx |= _part1by1_u64(x, NBITS).to(OUT_DTYPE)
        idx |= (_part1by1_u64(y, NBITS) << 1).to(OUT_DTYPE)
        tl.store(out_ptr + offsets, idx, mask=mask)


morton_encode_2d_kernel_autotuned = triton.autotune(
    configs=triton_autotune_configs(),
    key=["AUTOTUNE_KEY", "NBITS"],
)(morton_encode_2d_kernel)
...
def morton_encode_2d_triton(
    ...
    *,
    ...
    """Encode (x, y) to Morton indices using Triton."""
    ...
    if triton_tuning == "heuristic":
        ...
        morton_encode_2d_kernel[grid](  # type: ignore[reportIndexIssue]
            x,
            y,
            out,
            n_elements,
    ...
    morton_encode_2d_kernel_autotuned[grid](  # type: ignore[reportIndexIssue]
        x,
        y,
        out,
        n_elements,
